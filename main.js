...
const validateCred = (creditArr) => {
    ...
    let test = [1, 2, 3, 4].reduce(reducer)
    let ccCopy = creditArr.slice();  //Copy of array passed
    for (let i = ccCopy.length - 2; i >= 0; i -= 2) {
        ccCopy[i] *= 2;
        if (ccCopy[i] > 9) {
            ccCopy[i] -= 9
        }  
    }
    let cardSum = ccCopy.reduce(reducer) ;
      // let cardSum = ccCopy.reduce() ;
